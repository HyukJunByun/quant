...
class MyApp(QWidget):
    ...
    def run_script(self):
        ...
        # self.btn.setMaximum(len(code_data))
        ...
        for a in range(0, len(code_data)):
            ...
            if (code_num[0] != '9'):
                ...
                fics_data = BeautifulSoup(webpage.content, 'lxml',
                                          parse_only=SoupStrainer('span', {'class': "stxt stxt2"}))
                webpage.close()
                webpage = None
                ...
                if (no_fics != 'FICS  창업투자 및 종금'):
                    ...
                    fics_data.decompose()
                    try:
                        ifrs_table = pd.read_html(fnguide_url, match='Financial Highlight', flavor='lxml', header=0,
                                                  index_col=0, attrs={'class': 'us_table_ty1 h_fix zigbg_no'})
                        ...
                        if (len(DA.columns) == 8 and list(DA.index.values)[1] == '매출액'):  # 금융주 및 정보부족 회사 필터링
                            ...
                            if (DA.loc['ROE', [2, 3, 4]].isna().sum() == 0 and DA.loc['배당수익률', [3, 4]].isna().sum() == 0
                                ...
                                por = siga / (DQ[4]['영업이익(발표기준)'] + DQ[3]['영업이익(발표기준)'] + DQ[2]['영업이익(발표기준)'] + DQ[1][
                                    '영업이익(발표기준)'])
                                ...
                                if (pbr > 0.2 and boon_per > 2 and psr > 0.1 and por > 2 and asset_growth > 0):
                                    ...
                                    earn = \
                                    pd.read_html(fnguide_url, match='포괄손익계산서', flavor='lxml', header=0, index_col=0,
                                                 attrs={'class': 'us_table_ty1 h_fix zigbg_no'})[1]
                                    ...
                                    # GP/A
                                    cash_flow_table = \
                                    pd.read_html(fnguide_url, match='현금흐름표', flavor='lxml', header=0, index_col=0,
                                                 attrs={'class': 'us_table_ty1 h_fix zigbg_no'})[1]
                                    cash_flow = (pd.to_numeric(cash_flow_table.loc['영업활동으로인한현금흐름', recent_DQ],
                                                               errors='ignore') +
                                                 pd.to_numeric(cash_flow_table.loc['영업활동으로인한현금흐름', rrecent_DQ],
                                                               errors='ignore') +
                                                 pd.to_numeric(cash_flow_table.loc['영업활동으로인한현금흐름', rrrecent_DQ],
                                                               errors='ignore') +
                                                 pd.to_numeric(cash_flow_table.loc['영업활동으로인한현금흐름', rrrrecent_DQ],
                                                               errors='ignore'))
                                    # 영업현금흐름
                                    pcr = siga / cash_flow
                                    # PCR
                                    naver = (DQ[4]['영업이익(발표기준)'] + DQ[3]['영업이익(발표기준)'] + DQ[2]['영업이익(발표기준)'] + DQ[1][
                                        '영업이익(발표기준)'])
                                    # 영업이익
                                    ...
                                    if (pcr > 1 and naver > 0 and cash_flow > 0 and kakao > 0):
                                        ...
                                        # 재무비율
                                        mon_ratio = \
                                        pd.read_html(fnguide_url, match='재무비율', flavor='lxml', header=0, index_col=0,
                                                     attrs={'class': 'us_table_ty1 h_fix zigbg_no'})[0]
                                        owe = pd.to_numeric(mon_ratio.loc['순차입금비율계산에 참여한 계정 펼치기', recent_DA],
                                                            errors='ignore')
                                        # 순차입금비율
                                        mon_ratio = None
                                        if (math.isnan(owe) == True or owe < 200):
                                            ...
                                            # 투자지표
                                            invest_idea = \
                                            pd.read_html(fnguide_url, match='기업가치 지표', flavor='lxml', header=0,
                                                         index_col=0, attrs={'class': 'us_table_ty1 h_fix zigbg_no'})[0]
                                            bay_trend = pd.to_numeric(
                                                invest_idea.loc['배당성향(현금)(%)계산에 참여한 계정 펼치기', recent_DA],
                                                errors='ignore')
                                            # 배당성향
                                            ev_ebita = pd.to_numeric(
                                                invest_idea.loc['EV/EBITDA계산에 참여한 계정 펼치기', recent_DA], errors='ignore')
                                            # EV/EBITA
                                            ...
                                            if (pfcr > 1):
                                                ...
                                                for i in range(0, 14):
                                                    ...
                                                    cell_col = 66
                                                    for t in range(0, 8):
                                                        wb_data.range(chr(cell_col) + str(i + 27)).value = DA[t][
                                                            table_content[i]]
                                                        cell_col += 1
                                                for i in range(0, 12):
                                                    # 연결-분기 데이터 엑셀에 넣기
                                                    cell_col = 66
                                                    for t in range(0, 8):
                                                        wb_data.range(chr(cell_col) + str(i + 51)).value = DQ[t][
                                                            table_content[i]]
                                                        cell_col += 1
                                                for i in range(0, 4):
                                                    ...
                                                    cell_col = 66
                                                    for t in range(0, 8):
                                                        wb_data.range(chr(cell_col) + str(i + 69)).value = BQ[t][
                                                            table_content[i]]
                                                        cell_col += 1
                                                for i in range(0, 3):
                                                    ...
                                                    cell_col = 66
                                                    for t in range(0, 8):
                                                        wb_data.range(chr(cell_col) + str(i + 73)).value = BQ[t][
                                                            table_content[i + 6]]
                                                        cell_col += 1
                                                for t in range(0, 8):
                                                    # 개별-분기 데이터 엑셀에 넣기(자본금)
                                                    wb_data.range(chr(t + 66) + str(76)).value = BQ[t][
                                                        table_content[11]]
